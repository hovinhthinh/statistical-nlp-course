...
def count_transitions(taggedWords):
    ...
    # Initialize countEmissionsOfWords with empty dicts for each word
    ...
    for i in range(len(taggedWords) - 1):
        countPostagsPerPostag[taggedWords[i][1]] += 1
        if taggedWords[i+1][1] not in countTransitionsOfPostags[taggedWords[i][1]]:
            countTransitionsOfPostags[taggedWords[i][1]][taggedWords[i+1][1]] = 0
        countTransitionsOfPostags[taggedWords[i][1]][taggedWords[i + 1][1]] += 1
...
def count_emissions(taggedWords):
    ...
    for i in range(len(taggedWords)):
        countWordsPerPostag[taggedWords[i][1]] += 1
        if taggedWords[i][0] not in countEmissionsOfPostags[taggedWords[i][1]]:
            countEmissionsOfPostags[taggedWords[i][1]][taggedWords[i][0]] = 0
        countEmissionsOfPostags[taggedWords[i][1]][taggedWords[i][0]] += 1
...
def get_transition_probability(previousPostag, countDataTransitions):
    ...
    for tag in countPostagsPerPostag:
        probabilityTransitions[tag] = countTransitionsOfPostags[previousPostag].get(tag, 0) / countPostagsPerPostag[previousPostag]
...
if __name__ == '__main__':
    ...
    testWords,testPostags = tuple(zip(*taggedWords['development']))
    ...
    if plottingEnabled:
        def evaluate_epsilon(epsilon):
            ''' Fill this function '''
            MCR, MCRRatioOOV, sentences, logProbability = evaluate_HMM_for_epsilon(testWords, testPostags, OOVtest,
                                                                                   epsilon, countSentences=True)
            return MCR
